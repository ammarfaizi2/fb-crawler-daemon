...
class sentistrength:
    def __init__(self, config=dict()):
        sentistrength_location = os.path.dirname(os.path.realpath(__file__))
        cwd = os.getcwd()

        base_location = None if cwd == sentistrength_location else sentistrength_location + "/"

        self.negasi = [line.replace('\n','') for line in open(base_location + "negatingword.txt").read().splitlines()]
        self.tanya = [line.replace('\n','') for line in open(base_location + "questionword.txt").read().splitlines()]
        #create sentiment words dictionary
        self.sentiwords_txt = [line.replace('\n','').split(":") for line in open(base_location + "sentiwords_id.txt").read().splitlines()]
